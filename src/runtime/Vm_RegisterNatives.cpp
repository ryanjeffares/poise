...
auto Vm::registerFloatNatives() noexcept -> void
{
    m_nativeFunctionLookup.emplace(m_nativeNameHasher("__NATIVE_FLOAT_POW"), NativeFunction{2, [] (std::span<Value> args) -> Value {
        ...
        return static_cast<f64>(std::pow(args[0].value<f64>(), args[1].type() == types::Type::Float ? args[1].value<f64>() : static_cast<f64>(args[1].value<i64>())));
    }});
...
{
    m_nativeFunctionLookup.emplace(m_nativeNameHasher("__NATIVE_LIST_APPEND"), NativeFunction{2, [](std::span<Value> args) -> Value {
        throwIfWrongType(0, args[0], types::Type::List);
        args[0].object()->asList()->append(std::move(args[1]));
        return Value::none();
    }});

    m_nativeFunctionLookup.emplace(m_nativeNameHasher("__NATIVE_LIST_INSERT"), NativeFunction{3, [](std::span<Value> args) -> Value {
        throwIfWrongType(0, args[0], types::Type::List);
        throwIfWrongType(1, args[1], types::Type::Int);
        return args[0].object()->asList()->insert(args[1].value<usize>(), std::move(args[2]));
    }});

    m_nativeFunctionLookup.emplace(m_nativeNameHasher("__NATIVE_LIST_REMOVE"), NativeFunction{2, [](std::span<Value> args) -> Value {
        throwIfWrongType(0, args[0], types::Type::List);
        return args[0].object()->asList()->remove(args[1]);
    }});

    m_nativeFunctionLookup.emplace(m_nativeNameHasher("__NATIVE_LIST_REMOVE_FIRST"), NativeFunction{2, [](std::span<Value> args) -> Value {
        throwIfWrongType(0, args[0], types::Type::List);
        return args[0].object()->asList()->removeFirst(args[1]);
    }});

    m_nativeFunctionLookup.emplace(m_nativeNameHasher("__NATIVE_LIST_REMOVE_AT"), NativeFunction{2, [](std::span<Value> args) -> Value {
        throwIfWrongType(0, args[0], types::Type::List);
        throwIfWrongType(1, args[1], types::Type::Int);
