...
{
    if (!m_memberVariables.contains(memberNameHash)) {
        throw Exception{
    ...
    return m_memberVariables.find(memberNameHash).value;
}
...
auto StructInstance::assignMember(usize memberNameHash, runtime::Value value) -> void
{
    ...
    m_memberVariables.update(MemberVariable{
...
auto StructInstance::toString() const noexcept -> std::string
{
    ...
    for (auto i = 0_uz; i < memberVariables.size(); i++) {
        ...
        const auto& value = m_memberVariables.find(nameHash).value;
...
auto StructInstance::findObjectMembers(std::unordered_set<Object*>& objects) const noexcept -> void
{
    const auto structType = m_structType.object()->asStruct();

    for (const auto memberVariables = structType->memberVariables(); const auto& memberVariable : memberVariables) {
        const auto& value = m_memberVariables.find(memberVariable.nameHash).value;
...
auto StructInstance::removeObjectMembers() noexcept -> void
{
    ...
    for (const auto memberVariables = structType->memberVariables(); const auto& memberVariable : memberVariables) {
        auto& value = m_memberVariables.find(memberVariable.nameHash).value;
